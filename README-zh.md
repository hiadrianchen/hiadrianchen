...
- **不会自我印证的多 Agent 系统**：让不同厂商的模型相互验证，以证据而非票数为准，避免几个 Agent 彼此附和就当作达成了共识。
- **个人的 AI 操作系统**：把笔记、任务、内容，以及那些长期推进的事务，沉淀为能够留存的文件、待办与记忆，而不是随着一轮对话结束而消失。
- **可延续的 Agent 上下文**：用 Git 管理记忆与上下文，让 Agent 在更换设备、间隔一段时间后重新接手时，无需从头交代。
- **把信息转化为知识**：将文章、视频、仓库、零散笔记汇入其中，产出可检索、可复用的内容。
